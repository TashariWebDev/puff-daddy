...
if (! function_exists('percentage_saved')) {
    function percentage_saved($price, $old_price): string
    {
        $amountSaved = (int) $old_price - (int) $price;

        if ($amountSaved > 0) {
            return (int) number_format(($amountSaved / $price) * 100, 2).'%';
        }
